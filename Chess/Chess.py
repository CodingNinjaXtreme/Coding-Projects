...
WINDOW_SIZE = 640
FPS = 60
ASSET_DIR = Path(__file__).parent  # images next to Chess.py
AI_ENABLED = True
AI_COLOR = 'b'
AI_DEPTH = 2
...
class Game:
    ...
    def draw(self, moving_piece=None, piece_pos=None):
        ...
        if moving_piece and piece_pos:
            img = self.images.get(moving_piece)
            if img:
                pos = (int(piece_pos[0]), int(piece_pos[1]))
                self.screen.blit(img, pos)
            else:
                px,py = int(piece_pos[0]), int(piece_pos[1])
                center = (px + self.square//2, py + self.square//2)
                color = (255,255,255) if moving_piece[0]=='w' else (0,0,0)
                pygame.draw.circle(self.screen, color, center, self.square//3)
        ...
        if self.game_over:
            over_txt = f"Game Over! {'White' if self.winner=='w' else 'Black'} wins"
            self.screen.blit(self.font.render(over_txt, True, (255,0,0)), (6,46))
        ...
        if self.selected:
            if (r,c) in self.legal_moves:
                ...
                with self.lock:
                    ...
                    captured,promoted,piece=self.board.push_move(sr,sc,r,c)
    ...
    def animate_move(self,sr,sc,tr,tc,piece,steps=6):
        ...
        for step in range(1,steps+1):
            ...
            self.draw(moving_piece=piece,piece_pos=(int(interp_x),int(interp_y)))
            pygame.display.flip()
            self.clock.tick(FPS)
